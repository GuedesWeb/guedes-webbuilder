...
// Define window.WB_EDITOR_SRC: string IIFE injetada no iframe do preview
window.WB_EDITOR_SRC = '(function(){'
+ 'if(window.parent===window)return;'
+ 'var activeEl=null,activeOverlay=null;'
...
+   'var chips=document.querySelectorAll(".wb-edit-chip");for(var i=0;i<chips.length;i++)chips[i].remove();'
+ '}'

// Extrai texto limpo: remove SVG, script, style, &nbsp;
...
// Posiciona o popup relativo ao elemento
...
+   'el.classList.add("wb-editing");activeEl=el;'
...
+   'chip.textContent="✏️";'
+   'chip.style.cssText="position:fixed;z-index:99998;top:"+(r.top-8)+"px;left:"+(r.right-8)+"px;'
+     +'"background:#4C43F7;color:#fff;font-size:11px;width:22px;height:22px;display:flex;align-items:center;justify-content:center;'
+     +'"border-radius:50%;pointer-events:none;box-shadow:0 2px 8px rgba(76,67,247,.5);transition:opacity .12s";'
...
// ── Eventos de Hover ── (AMPliados: agora inclui SPAN, A, e qualquer elemento com data-editable)
// Prioridade: data-editable-text > img[data-editable] > [data-editable]
+ 'document.addEventListener("mouseover",function(e){'
+   // 1. data-editable-text (texto explícito)
+   'var tel=e.target.closest("[data-editable-text]");'
+   'if(tel){'
+     'tel.classList.add("wb-hover");'
+     'if(window.getComputedStyle(tel).display!=="none"&&tel.offsetHeight>0)showChip(tel);'
+     'return;'
+   '}'
+   // 2. img[data-editable] (imagem)
+   'var iel=e.target.closest("img[data-editable]");'
+   'if(iel){'
+     'iel.classList.add("wb-hover-img");'
+     'showChip(iel);'
+     'return;'
+   '}'
+   // 3. [data-editable] — qualquer elemento, exceto IMG (já tratado) e elementos que só contêm img filho
+   'var dl=e.target.closest("[data-editable]");'
+   'if(dl&&dl.tagName!=="IMG"){'
+     // Pula se o elemento só tem imagem dentro (ex: div wrapper de card com bg-image)
+     'var txt=getEditableText(dl);'
+     'if(txt||dl.tagName==="A"){'
+       'dl.classList.add("wb-hover");'
+       'if(window.getComputedStyle(dl).display!=="none"&&dl.offsetHeight>0)showChip(dl);'
+     '}'
+   '}'
+ '});'
...
// ── Eventos de Click ── (AMPliado: SPAN e A agora são editáveis)
+ 'document.addEventListener("click",function(e){'
+   // Ignora cliques dentro do popup
+   'if(e.target.closest(".wb-editor-popup"))return;'
+   'closeEditor();'

+   // 1. data-editable-text (prioridade máxima)
+   'var tel=e.target.closest("[data-editable-text]");'
+   'if(tel){'
+     'e.preventDefault();e.stopPropagation();'
+     'openTextEditor(tel,tel.getAttribute("data-editable-text"));'
+     'return;'
+   '}'
...
+   'if(iel){'
+     'e.preventDefault();e.stopPropagation();'
+     'openImageEditor(iel,iel.getAttribute("data-editable"));'
+     'return;'
+   '}'

+   // 3. [data-editable] em qualquer elemento (REMOVENDO as restrições de SPAN e A)
+   // Só pula IMG e elementos que contêm apenas img filho sem texto
+   'var dl=e.target.closest("[data-editable]");'
+   'if(dl&&dl.tagName!=="IMG"){'
+     'var hasImgChild=dl.querySelector("img");'
+     'var txt=getEditableText(dl);'
+     // Se tem texto editável OU é um link sem data-editable-text separado
...
+   // 4. Intercepta navegação de links dentro do preview
...
// ── Listener para refresh in-place (vindo do parent) ──
+ 'window.addEventListener("message",function(e){'
+   'if(!e.data||!e.data.type)return;'
+   'if(e.data.type==="wb-refresh-text"){'
+     // Atualiza todos os elementos com essa chave (pode ter múltiplos data-editable-text)
